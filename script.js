...
var groundContainer = document.getElementsByClassName("container")[0];
var size = 10;
var score1 = 0;
var score2 = 0;
var currentRod = rod2;
...
// important functions, which required to play game
newGameButton.addEventListener("click", newGame);
startButton.addEventListener("click", visibleScreen);
...
function newGame() {
  ...
  document.getElementById("body-container").classList.add("change-opacity");
...
// when user click on start game button, screen visible completely
function visibleScreen() {
  document.getElementById("body-container").classList.remove("change-opacity");
...
function moveRod(event) {
  ...
  var containerW = groundContainer.clientWidth;

  console.log(event.keyCode);
  if (key === 68) {
    //pressed D, for move rod1 to right direction
    ...
    //pressed A, for move rod1 to left direction
    if (r1Left - size >= 0) {
      rod1.style.left = r1Left - size + "px";
      r1Left -= size;
    } else {
...
//to move ball within ground container
function setBallPosition() {
  var ballTop = ball.offsetTop;
  var ballLeft = ball.offsetLeft;
  var ballW = ball.offsetWidth;

  //when ball touches right boundary of the ground container
  if (ballLeft + ballW === groundContainer.clientWidth)
    //reversing its x coordinates
    xDirection *= -1;
  else if (ball.offsetLeft === 0) xDirection *= -1;
  //when ball lies at the top of the rod1, check if it strikes the rod or not
  else if (notIntial && ballTop === rod1.offsetHeight) {
    var rodLeft = rod1.offsetLeft - ball.offsetWidth;
    var rodRight = rod1.offsetLeft + rod1.offsetWidth;

    if (ballLeft <= rodRight && ballLeft >= rodLeft) {
      yDirection *= -1;
      score1++;
      scoreDisp1.innerText = score1 + "";
    } else {
      alert("Ohh No You Missed it!!");
      ...
      setGame();
      if (livesNo1 === 0) {
        if (score1 > score2) alert("Hurey...Winner is player 1");
        else if (score2 > score1) alert("Hurrey...Winner is player 2");
    ...
    notIntial &&
    ballTop + ballW === groundContainer.clientHeight - rod2.offsetHeight
  ) {
    var rodLeft = rod2.offsetLeft - ball.offsetWidth;
    var rodRight = rod2.offsetLeft + rod2.offsetWidth;
    ...
    if (ballLeft <= rodRight && ballLeft >= rodLeft) {
      ...
      setGame();
      if (livesNo2 === 0) {
        if (score1 > score2) alert("Hurrey...Winner is player 1");
        else if (score2 > score1) alert("Hurrey...Winner is player 2 ");
        else alert("It's a Tie Game");
        newGame();
      }
